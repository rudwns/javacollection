...
/*====== 생성자 ======*/
	...
	// 생성자를 만들면 디폴트를 별도로 추가해줘야 함
	// setter 대신에 생성자로 학번,이름,3과목점수를 점수를 입력 받음
	public Grade(String name,String hakbun,int kor,int eng,int math) {
		this.hakbun = hakbun;
		this.name = name;
		this.eng = eng;
		this.math= math;
...
/*======멤버 메소드========*/
	...
	public int getTotal() {
		...
		total = this.eng+this.math+this.kor;
		return total;
	}
	/*
	 ...
	 */
	public int getTotal(int kor, int math, int eng){
		int total = 0;
		total = kor + eng + math;
		// 위 kor, eng, math 는 this 를 지움으로써
		// 멤버필드에 인스턴스 변수가 아닌
		// 파라미터로 넘어 온 지역 변수가 되었다.
		return total;
	}
